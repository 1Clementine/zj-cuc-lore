...
def main():
    ...
    csv_path = os.path.join(results_dir, 'performance',
                            'lore_sm3_kem_cycles_time_all_levels.csv')
    summary_dir = os.path.join(results_dir, 'summary')
    os.makedirs(summary_dir, exist_ok=True)

    formal_repo = os.environ.get('LORE_SM3_REPO', '')
    ...
    out = os.path.join(summary_dir, 'LORE_SM3_KEM_CYCLES_TIME_SUMMARY.md')
    with open(out, 'w') as f:
        f.write("""# Lore-SM3 KEM Cycles and Time Benchmark
...
""")
    ...
    if formal_report_dir:
        dest = os.path.join(formal_report_dir, 'SM3_KEM_ALL_LEVELS_PERFORMANCE.md')
