# How to run Lore-SHAKE KEM benchmark on a new server

## 1. Unpack

```bash
tar -xzf lore_shake_bench_bundle_YYYYMMDD_HHMMSS.tar.gz
cd lore_shake_bench_bundle_YYYYMMDD_HHMMSS
```
...
2. Check source purity (pure SHAKE, no SM3 wrapper)
...
- `zj-cuc-lore/report/performance/lore_shake_kem_cycles_time_all_levels.csv`
- `zj-cuc-lore/report/LORE_SHAKE_ALL_LEVELS_PERFORMANCE.md`
...
- The SHAKE256 formal branch only retains Lore-L4. The all-levels worktree (`zj-cuc-lore-shake-all-levels-bench/`) contains Lore-L1 through Lore-L4.
